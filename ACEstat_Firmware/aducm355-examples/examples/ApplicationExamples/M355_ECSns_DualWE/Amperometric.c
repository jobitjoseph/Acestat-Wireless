...
AD5940Err AppECSnrInit(AppECSnsCfg_Type *SnsCfg, uint8_t SnsSel)
{
  ...
  if(SnsSel == M355_CHAN0)
  {
    lp_loop.LpAmpCfg.LpTiaSW = LPTIASW(5)|LPTIASW(2)|LPTIASW(4)|LPTIASW(12)|LPTIASW(13)|LPTIASW(13); 
    lp_loop.LpAmpCfg.LpPaPwrEn = bTRUE;
    lp_loop.LpDacCfg.PowerEn = bTRUE;
}
  else
  {
    lp_loop.LpAmpCfg.LpTiaSW = LPTIASW(5)|LPTIASW(8)|LPTIASW(12)|LPTIASW(13)|LPTIASW(13);
    /* Power down LPDAC1 and LPAMP1 */
    lp_loop.LpAmpCfg.LpPaPwrEn = bFALSE;
    lp_loop.LpDacCfg.PowerEn = bFALSE;
  }
...
/* Configure sequence to measure both channels on M355.
   Could be broken into 2 seperate sequenes alternatively
*/
AD5940Err AppM355SeqMeasureGen(void)
{
  ...
  clks_cal.DataCount = 8;               /* Take average of 8 samples */
...
static AD5940Err AppAMPDataProcess(int32_t * const pData, uint32_t *pDataCount)
{
  ...
  for(i = 0; i< NumData; i++)
  {
    for(j = 0; j<AppAmpCfg.NumSamplesAvg; j++)
    {
      ...
      AverageCh1 += pData[j+AppAmpCfg.NumSamplesAvg] & 0xffff;      
    }
    average = AverageCh0/AppAmpCfg.NumSamplesAvg;
    average1 = AverageCh1/AppAmpCfg.NumSamplesAvg; 
    AppAmpCfg.SensorCh0.ResultBuffer[i] = AppAMPCalcCurrent(AverageCh0/AppAmpCfg.NumSamplesAvg, AppAmpCfg.SensorCh0.RtiaCalValue.Magnitude);
    AppAmpCfg.SensorCh1.ResultBuffer[i] = AppAMPCalcCurrent(AverageCh1/AppAmpCfg.NumSamplesAvg, AppAmpCfg.SensorCh1.RtiaCalValue.Magnitude);
  }
  *pDataCount = NumData;
  return AD5940ERR_OK;
}
