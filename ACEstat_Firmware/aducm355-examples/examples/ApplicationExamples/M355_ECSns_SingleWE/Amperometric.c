...
static AD5940Err AppAMPDataProcess(int32_t * const pData, uint32_t *pDataCount)
{
  ...
  for(i=0; i<datacount; i++)
    { 
      pData[i] &= 0xffff;
    //  printf("%d ", pData[i]);
      AppAmpCfg.SensorCh0.ResultBuffer[i] = AppAMPCalcCurrent(pData[i], AppAmpCfg.SensorCh0.RtiaCalValue.Magnitude);
     // AppAmpCfg.SensorCh0.ResultBuffer[i] = AppAMPCalcVoltage(pData[i]);
    }
  return AD5940ERR_OK;
}
